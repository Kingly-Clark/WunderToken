...
import { accessControlMessage } from "./utils/strings";

describe("Roles", () => {
  describe("Granting", () => {
    it("Should be able to grant MINTER_ROLE as DEFAULT_ADMIN_ROLE", async () => {
      // confirm owner has DEFAULT_ADMIN_ROLE
      const { wunderTokenV1, owner, notOwner } = await loadFixture(deployWunderTokenV1);
      ...
      expect(await wunderTokenV1.hasRole(await wunderTokenV1.MINTER_ROLE(), notOwner.address)).to.be.false;

      // grant MINTER_ROLE to notOwner
      await wunderTokenV1.grantRole(await wunderTokenV1.MINTER_ROLE(), notOwner.address);

      // confirm notOwner has MINTER_ROLE
      expect(await wunderTokenV1.hasRole(await wunderTokenV1.MINTER_ROLE(), notOwner.address)).to.be.true;
    });
    ...
    it("Should be able to grant BURNER_ROLE as DEFAULT_ADMIN_ROLE", async () => {
      ...
      expect(await wunderTokenV1.hasRole(await wunderTokenV1.BURNER_ROLE(), notOwner.address)).to.be.false;

      // grant BURNER_ROLE to notOwner
      await wunderTokenV1.grantRole(await wunderTokenV1.BURNER_ROLE(), notOwner.address);

      // confirm notOwner has BURNER_ROLE
      expect(await wunderTokenV1.hasRole(await wunderTokenV1.BURNER_ROLE(), notOwner.address)).to.be.true;
    });
    ...
    it("Should be able to grant GOVERN_ROLE as DEFAULT_ADMIN_ROLE", async () => {
      ...
      expect(await wunderTokenV1.hasRole(await wunderTokenV1.GOVERN_ROLE(), notOwner.address)).to.be.false;

      // grant GOVERN_ROLE to notOwner
      await wunderTokenV1.grantRole(await wunderTokenV1.GOVERN_ROLE(), notOwner.address);

      // confirm notOwner has GOVERN_ROLE
      expect(await wunderTokenV1.hasRole(await wunderTokenV1.GOVERN_ROLE(), notOwner.address)).to.be.true;
    });
    ...
    it("Should be able to grant PAUSER_ROLE as DEFAULT_ADMIN_ROLE", async () => {
      ...
      expect(await wunderTokenV1.hasRole(await wunderTokenV1.PAUSER_ROLE(), notOwner.address)).to.be.false;

      // grant PAUSER_ROLE to notOwner
      await wunderTokenV1.grantRole(await wunderTokenV1.PAUSER_ROLE(), notOwner.address);

      // confirm notOwner has PAUSER_ROLE
      expect(await wunderTokenV1.hasRole(await wunderTokenV1.PAUSER_ROLE(), notOwner.address)).to.be.true;
    });
  ...
  describe("Revoking", () => {
    it("Should be able to revoke MINTER_ROLE as DEFAULT_ADMIN_ROLE", async () => {
      ...
      await wunderTokenV1.revokeRole(await wunderTokenV1.MINTER_ROLE(), notOwner.address);
    ...
    it("Should be able to revoke BURNER_ROLE as DEFAULT_ADMIN_ROLE", async () => {
      ...
      await wunderTokenV1.revokeRole(await wunderTokenV1.BURNER_ROLE(), notOwner.address);
    ...
    it("Should be able to revoke GOVERN_ROLE as DEFAULT_ADMIN_ROLE", async () => {
      ...
      await wunderTokenV1.revokeRole(await wunderTokenV1.GOVERN_ROLE(), notOwner.address);
    ...
    it("Should be able to revoke PAUSER_ROLE as DEFAULT_ADMIN_ROLE", async () => {
      ...
      await wunderTokenV1.revokeRole(await wunderTokenV1.PAUSER_ROLE(), notOwner.address);
  ...
  describe("Methods", () => {
    describe("MINTER_ROLE", () => {
      it("Should be able to `mint` Wunder as MINTER_ROLE", async () => {
        const { wunderTokenV1, owner, minter } = await loadFixture(deployWunderTokenV1);
        // confirm minter doens't have MINTER_ROLE
        expect(await wunderTokenV1.hasRole(await wunderTokenV1.MINTER_ROLE(), minter.address))
          .to
          .be
          .false;
        ...
        expect(await wunderTokenV1.hasRole(await wunderTokenV1.MINTER_ROLE(), minter.address)).to.be.true;
      ...
      it("Shouldn't be able to `mint` Wunder as not MINTER_ROLE", async () => {
        ...
        // confirm minter doens't have MINTER_ROLE
        expect(await wunderTokenV1.hasRole(await wunderTokenV1.MINTER_ROLE(), minter.address)).to.be.false;

        // mint 100 Wunder to minter
        await expect(wunderTokenV1
          .connect(minter)
          .mint(minter.address, 100))
    ...
    describe("BURNER_ROLE", () => {
      ...
      it("Should be able to `burn` Wunder as BURNER_ROLE", async () => {
        const { wunderTokenV1, owner, burner } = await loadFixture(deployWunderTokenV1);

        // confirm burner doens't have BURNER_ROLE
        expect(await wunderTokenV1.hasRole(await wunderTokenV1.BURNER_ROLE(), burner.address))
          .to
          .be
          .false;

        // grant BURNER_ROLE to burner
        await wunderTokenV1.connect(owner).grantRole(await wunderTokenV1.BURNER_ROLE(), burner.address);
        ...
        await wunderTokenV1.connect(owner).grantRole(await wunderTokenV1.MINTER_ROLE(), burner.address);
      ...
      it("Shouldn't be able to `burn` Wunder as not BURNER_ROLE", async () => {
        ...
        // burn 100 Wunder from burner
        await expect(wunderTokenV1.connect(burner).burn(100))
          .to
          .be
    ...
    describe("GOVERN_ROLE", () => {
      ...
      it("Should be able to `freeze` an account as GOVERN_ROLE", async () => {
        const { wunderTokenV1, owner, governor, acc1 } = await loadFixture(deployWunderTokenV1);
        // confirm governor doens't have GOVERN_ROLE
        expect(await wunderTokenV1.hasRole(await wunderTokenV1.GOVERN_ROLE(), governor.address))
          .to
          .be
          .false;

        // grant GOVERN_ROLE to governor
        await wunderTokenV1.connect(owner).grantRole(await wunderTokenV1.GOVERN_ROLE(), governor.address);

        // confirm governor has GOVERN_ROLE
        expect(await wunderTokenV1.hasRole(await wunderTokenV1.GOVERN_ROLE(), governor.address)).to.be.true;
      ...
      it("Shouldn't be able to `freeze` an account as not GOVERN_ROLE", async () => {
        const { wunderTokenV1, governor, acc1 } = await loadFixture(deployWunderTokenV1);
        // confirm governor doens't have GOVERN_ROLE
        expect(await wunderTokenV1.hasRole(await wunderTokenV1.GOVERN_ROLE(), governor.address))
          .to
          .be
        ...
        // freeze acc1
        await expect(wunderTokenV1.connect(governor).freeze(acc1.address))
          .to
          .be
          .revertedWithCustomError(
            wunderTokenV1,
      ...
      it("Should be able to `unfreeze` an account as GOVERN_ROLE", async () => {
        const { wunderTokenV1, owner, governor, acc1 } = await loadFixture(deployWunderTokenV1);
        // confirm governor doens't have GOVERN_ROLE
        expect(await wunderTokenV1.hasRole(await wunderTokenV1.GOVERN_ROLE(), governor.address)).to.be.false;
      ...
      it("Shouldn't be able to `unfreeze` an account as not GOVERN_ROLE", async () => {
        ...
        // unfreeze acc1
        await expect(wunderTokenV1.connect(governor).unfreeze(acc1.address))
          .to
          .be
          .revertedWithCustomError(
            wunderTokenV1,
      ...
      it("Should be able to `seize` an account as GOVERN_ROLE", async () => {
        const { wunderTokenV1, owner, governor, minter, acc1 } = await loadFixture(deployWunderTokenV1);
        // confirm governor doens't have GOVERN_ROLE
        expect(await wunderTokenV1.hasRole(await wunderTokenV1.GOVERN_ROLE(), governor.address)).to.be.false;
        // grant GOVERN_ROLE to governor
        await wunderTokenV1.connect(owner).grantRole(await wunderTokenV1.GOVERN_ROLE(), governor.address);
        ...
        // grant MINTER_ROLE to minter
        await wunderTokenV1.connect(owner).grantRole(await wunderTokenV1.MINTER_ROLE(), minter.address);
        ...
        expect(await wunderTokenV1.balanceOf(await wunderTokenV1.getAddress())).to.equal(0);
      ...
      it("Shouldn't be able to `seize` an account as not GOVERN_ROLE", async () => {
        const { wunderTokenV1, governor, minter, acc1 } = await loadFixture(deployWunderTokenV1);
        // confirm governor doens't have GOVERN_ROLE
        expect(await wunderTokenV1.hasRole(await wunderTokenV1.GOVERN_ROLE(), governor.address)).to.be.false;

        // seize acc1
        await expect(wunderTokenV1.connect(governor).seize(acc1.address))
          .to
          .be
          .revertedWithCustomError(
            wunderTokenV1,
      ...
      it("Shouldn't be able to `withdraw` Wunder as not GOVERN_ROLE", async () => {
        ...
        // withdraw 100 Wunder
        await expect(wunderTokenV1.connect(governor).withdraw(100))
          .to
          .be
          .revertedWithCustomError(
            wunderTokenV1,
    ...
    describe("PAUSER_ROLE", () => {
      ...
      it("Should be able to `pause` as PAUSER_ROLE", async () => {
        const { wunderTokenV1, owner, pauser } = await loadFixture(deployWunderTokenV1);
        // confirm pauser doens't have PAUSER_ROLE
        expect(await wunderTokenV1.hasRole(await wunderTokenV1.PAUSER_ROLE(), pauser.address)).to.be.false;

        // grant PAUSER_ROLE to pauser
        await wunderTokenV1.connect(owner).grantRole(await wunderTokenV1.PAUSER_ROLE(), pauser.address);

        // confirm pauser has PAUSER_ROLE
        expect(await wunderTokenV1.hasRole(await wunderTokenV1.PAUSER_ROLE(), pauser.address)).to.be.true;
      ...
      it("Shouldn't be able to `pause` as not PAUSER_ROLE", async () => {
        ...
        // pause wunderTokenV1
        await expect(wunderTokenV1.connect(pauser).pause())
          .to
          .be
          .revertedWithCustomError(
            wunderTokenV1,
      ...
      it("Shouldn't be able to `unpause` as not PAUSER_ROLE", async () => {
        ...
        // unpause wunderTokenV1
        await expect(wunderTokenV1.connect(pauser).unpause())
          .to
          .be
          .revertedWithCustomError(
            wunderTokenV1,
