...
describe("V1", () => {
  describe("GAS Usage", () => {
    it("Evaluate GAS costs for 10 transactions of different sizes between acc1, acc2 and acc3", async () => {
      const { wunderTokenV1, acc1, acc2, acc3 } = await loadFixture(
      ...
      expect(await wunderTokenV1.isFrozen(acc3.address)).to.be.false()
      for (let i = 0; i < 10; i++) {
        const amount = Math.floor(Math.random() * 1000)
        await wunderTokenV1.connect(acc1).transfer(acc2.address, amount)
        await wunderTokenV1.connect(acc2).transfer(acc3.address, amount)
        await wunderTokenV1.connect(acc3).transfer(acc1.address, amount)
      }
    })
  })
})
